...
export async function POST(request: Request) {
  try {
    ...
    const customerEmail = invoice.user?.email || `invoice-${invoice.invoiceNumber}@customer.local`;
...
async function createVoucherPayment(order: any, gateway: string) {
  ...
  const customerEmail = order.customerEmail || `order-${order.orderNumber}@customer.local`;
